<?php
    ...
    if($check_categoria->rowCount()<=0){
        echo '
            <div class="notification is-danger is-light">
                <strong>¡Ocurrio un error inesperado!</strong><br>
                EL Equipo seleccionada no existe
            </div>
        ';
        exit();
    }
	...
	if($_FILES['producto_foto']['name']!="" && $_FILES['producto_foto']['size']>0){
        ...
        if(!file_exists(filename: $img_dir)){
            if(!mkdir(directory: $img_dir,permissions: 0777)){
                echo '
                    <div class="notification is-danger is-light">
                        <strong>¡Ocurrio un error inesperado!</strong><br>
    ...
    $guardar_producto->execute(params: $marcadores);

    if($guardar_producto->rowCount()==1){
        echo '
            <div class="notification is-info is-light">
                <strong>¡DESCRIPCIÓN REGISTRADA!</strong><br>
                La descripción se registro con exito
            </div>
        ';
    }else{

    	if(is_file(filename: $img_dir.$foto)){
			chmod(filename: $img_dir.$foto, permissions: 0777);
			unlink(filename: $img_dir.$foto);
        }

        echo '
            <div class="notification is-danger is-light">
                <strong>¡Ocurrio un error inesperado!</strong><br>
                No se pudo registrar la descripción, por favor intente nuevamente
            </div>
        ';
    }
